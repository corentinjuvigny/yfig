...
**======================================================================
 *$Software: YFIG2DEV
 *      $Id: gen-pdf.c 377 2018-12-09 22:39:34Z ia $
 * $HeadURL: http://lunix120.ensiie.fr/yfig/trunk/yfig2dev/gen-pdf.c $
 *  $Author: Ivan Auge (Email: auge@ensiie.fr)
**======================================================================*/

/*======================================================================*/
/*= Convert the figure into PDF format.                                =*/
/*======================================================================*/
...
static TodPdfCfg gen_pdf_getPdfCfg (Tds*ds,CboxDble*srcBoxCm);

/*======================================================================*/
/*= Interface                                                          =*/

/* creation of PDF figure into a temporary file */
...
#if 0
    stream = tool_tempfile(&ds->pdfname);
#else
    ds->pdfname = tool_strdup("/tmp/tutu.pdf");
    stream = tool_fileOpenWO(NULL,ds->pdfname);
#endif
    if ( stream==NULL )
      tool_ftlerr_printf(NULL,
        "conversion aborted: unable to create and open a temporary file: %s", strerror(errno));

  /* conversion of figure in PDF into the stream temporary file */
    TboxDble  srcBoxCm = ds_getSrcBBCm(ds);
    TodPdfCfg cfg      = gen_pdf_getPdfCfg(ds,&srcBoxCm);
    cfg.cfg_stream  = stream;
    cfg.cfg_outFile = ds->outname;
    status = fig_scene_pdf(ds->scene,ds->wMsgdrv,stream,&cfg,srcBoxCm);
    fclose(stream);

    if ( status!=1 )
        tool_ftlerr_printf(NULL,"%s","conversion aborted: PDF generation of figure failed");
}

/*======================================================================*/
/*= Private utilities                                                  =*/

static void gen_pdf_getPaperSizeCm(Tds*ds, double*dx, double*dy)
{
    TboxDble       box;
...
static TodPdfCfg gen_pdf_getPdfCfg (Tds*ds,CboxDble*srcBoxCm)
{
    ...
    gen_pdf_getPaperSizeCm(ds,&paperDx,&paperDy);
