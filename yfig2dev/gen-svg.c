...
**======================================================================
 *$Software: YFIG2DEV
 *      $Id: gen-svg.c 387 2019-01-26 22:53:16Z cj $
 * $HeadURL: http://lunix120.ensiie.fr/yfig/trunk/yfig2dev/gen-svg.c $
 *  $Author: Ivan Auge (Email: auge@ensiie.fr)
**======================================================================*/

/*======================================================================*/
/*= Convert the figure into SVG format.                                =*/
/*======================================================================*/
...
static TodPdfCfg gen_svg_getPdfCfg (Tds*ds,CboxDble*srcBoxCm);

/*======================================================================*/
/*= Interface                                                          =*/

/* creation of SVG figure into a temporary file */
extern void generate_svg(Tds*ds)
{
  ...
  /* conversion of figure in SVG into the stream temporary file */
    TboxDble  srcBoxCm = ds_getSrcBBCm(ds);
    TodPdfCfg cfg      = gen_svg_getPdfCfg(ds,&srcBoxCm);
    cfg.cfg_stream  = stream;
    cfg.cfg_outFile = ds->outname;
    status = fig_scene_svg(ds->scene,ds->wMsgdrv,stream,&cfg,srcBoxCm);
    fclose(stream);

    if ( status!=1 )
        tool_ftlerr_printf(NULL,"%s","conversion aborted: SVG generation of figure failed");
}

/*======================================================================*/
/*= Private utilities                                                  =*/

static void gen_svg_getPaperSizeCm(Tds*ds, double*dx, double*dy)
{
    TboxDble       box;
...
static TodPdfCfg gen_svg_getPdfCfg (Tds*ds,CboxDble*srcBoxCm)
{
    ...
    gen_svg_getPaperSizeCm(ds,&paperDx,&paperDy);
