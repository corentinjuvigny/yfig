 ...
 *      $Id: fig-scene-svg.c 387 2019-01-26 22:53:16Z cj $
 * $HeadURL: http://lunix120.ensiie.fr/yfig/trunk/fig/fig-scene-svg.c $
...
extern int fig_scene_svg(Tfig_scene* sc, Tmsgdrv* msgdrv, FILE* stream,
                         TodPdfCfg*cfg, CboxDble srcBoxCm)
{
    // create the SVG output device
    Tfigure* fig   = sc->sc_dessin;
    TodSvg*  odSvg = odsvg_new(msgdrv,stream,cfg, Box_dx(srcBoxCm), Box_dy(srcBoxCm));
    Toutdev* od    = (Toutdev*) odSvg;
    ...
    if (od==0) return 0;
{
    ...
    for (i=0 ; i<od->resources->fontUsed->eleNb ; i++) {
        odpdf_addFont(odSvg, odres_getFontUsed(od->resources,i));
    } 
#endif
    ...
    if ( odsvg_doHeader(odSvg) < 0 )  return -3;
    //if ( svgwrite_colors(odSvg,fig)<0 ) return -3;
    if ( fig_scene_draw(sc)<0 ) return -3;
    if ( odsvg_doFooter(odSvg)<0 ) return -3;
    fig_scene_clrEGScache( sc );
